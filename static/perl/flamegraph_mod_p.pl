...
my $timemax;                    # (over16ride the) sum of the counts
...
<svg version="1.1" width="100%" height="100%" onload="init(evt)" onresize="resize(evt)" viewBox="0 0 $w $h" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
...
sub color_log_scale {
	...
	my ($r, $g, $b) = (255, 255, 255);
	my $logmax = log($max);
	my $logmin = abs(log(0.1));
	$logmin = abs(log($min)) if $min > 0.1;
	$logmax = $logmin if $logmin > $logmax;
	my $logvalue = $logmin;
	$logvalue = log($value) if $value > 0.1;
	if ($value > 1.0) {
		$g = $b = int(210 * ($logmax - $logvalue) / $logmax);
	} elsif ($value < 1.0) {
		$r = $b = int(210 * ($logmax + $logvalue) / $logmax);
	}
	return "rgb($r,$g,$b)";
}
...
sub flow {
	...
	for ($i = $len_a; $i >= $len_same; $i--) {
		...
		$Node{"$k;$v"}->{extime} = delete $Tmp{$k}->{extime};
...
foreach (@Data) {
	...
	for (split /;/, $stack) {
        chomp $_;
		my $len = @Data_order;
		my $len2 = keys %{$Data_order[$n]};
		#print "out: $stack $len $len2 $n\n";
		#push @Data_order, {} if $n + 1 > $len;
		if (not exists $Data_order[$n]{$_}) {
			$Data_order[$n]{$_} = $len2;
		#	print "out 1: $n $_ $len2\n"
		}
		$n = $n + 1;
	}
}
...
sub sort_stacks {
  #  my ($a,$b) = @_;
	...
	$b =~ /^(.*)\s+?(\d+(?:\.\d*)?)$/;
	my $s_b = $1;
	my @parts_a = split ";", $s_a;
	my @parts_b = split ";", $s_b;
	my $len_a = @parts_a;
	my $len_b = @parts_b;
	#print "stacks: $s_a $s_b\n";
	for my $n (0..$len_a-1) {
		return 1 if $n + 1 > $len_b;
		my $ca = $Data_order[$n]{$parts_a[$n]};
		my $cb = $Data_order[$n]{$parts_b[$n]};
		#print "count: $n $len_a $len_b $ca $cb\n";
		return 1 if $ca > $cb;
		return -1 if $ca < $cb;
	}
}
...
#foreach (sort sort_stacks @Data) {
	...
	if (defined $samples2) {
        ...
        } elsif ($operator eq 'inverse-diff') {
            $delta = $samples - $samples2;
        } elsif ($operator eq 'ratio') {
            $delta = $samples>0 ? $samples2 / $samples : 0;
        } elsif ($operator eq 'inverse-ratio' ) {
            $delta = $samples2>0 ? $samples / $samples2 : 0;
        }
		$maxdelta = abs($delta) if abs($delta) > $maxdelta;
		$lowerdelta = $delta if $delta < $lowerdelta;
		$upperdelta = $delta if $delta > $upperdelta;
	...
	$last = flow($last, [ '', split ";", $stack ], $time, $delta, $extime);
...
while (my ($id, $node) = each %Node) {
	...
	(my $extimne_txt = $extime) # add commas per perlfaq5
		=~ s/(^[-+]?\d+?(?=(?>(?:\d{3})+)(?!\d))|\G\d{3}(?=\d))/$1,/g;
	...
	if ($func eq "" and $depth == 0) {
		...
		my $pct = sprintf "%.4f", ((100 * $samples) / ($timemax * $factor));
		my $expct = sprintf "%.4f", $pct * $extime / ($etime - $stime);
		...
		unless (defined $delta) {
			$info = "$escaped_func (Inclusive:$samples_txt $countname, $pct%; Exclusive:$extimne_txt $countname, $expct%)";
		} else {
            if ($operator eq 'diff') {
                my $d = $negate ? -$delta : $delta;
                my $deltapct = sprintf "%.4f", ((100 * $d) / ($timemax * $factor));
                $deltapct = $d > 0 ? "+$deltapct" : $deltapct;
                $info = "$escaped_func (Inclusive:$samples_txt $countname, $pct%; Exclusive:$extimne_txt $countname, $expct%; Difference:$deltapct%)";
             } else {
                $info = "$escaped_func (Inclusive:$samples_txt $countname, $pct%; Exclusive:$extimne_txt $countname, $expct%; Ratio:$delta)";
             }
		}
	}
	...
	if ($func eq "-") {
		...
		if ($operator eq 'diff') {
		    ...
		    $color = color_log_scale($delta, $lowerdelta, $upperdelta);
		}
	} elsif ($palette) {
