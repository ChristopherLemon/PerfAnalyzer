...
sub remember_calls {
    my ($primary_event,$stack,$pid,$tid) = @_;
    if( $primary_event eq "cycles") {
	    my $id = $pid . "-" . $tid;
		...
		if (not exists $previous_stacks{$id}{$base} ) {
            $previous_stacks{$id}{$base} = "";
        }
	    if ($previous_stacks{$id}{$base} eq $stack) {
	    } elsif ( index($stack,"[kernel")<0 and index($stack,"[unknown]")<0) {
	        unwind_calls($previous_stacks{$id}{$base},$stack);
			$previous_stacks{$id}{$base} = $stack;
		}
    }
}

sub unwind_calls {
    my ($previous_stack,$stack) = @_;
	my $test_stack = "";
	my $call_count = 0;
    for (split /;/, $stack) {
        chomp $_;
		$test_stack .= "$_";
		my $n = index($previous_stack,$test_stack);
		#print "out: $previous_stack, $stack, $test_stack,$n \n";
		if( index($previous_stack,$test_stack)!=0 ) {
			remember_stack("calls", $test_stack, 1);
		#	print "update, $_\n";
		}
		$test_stack .= ";";
    }
...
while (defined($_ = <>)) {
	...
	if (m/^$/) {
        ...
        if ($trace_calls) {
            remember_calls($primary_event,join(";", @stack), $m_pid, $m_tid) if @stack;
        }
        if ($accumulate) {
