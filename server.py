        ...
        if load_profile_data or load_hpc_data or load_perf_data:
            ...
            for foundfile in allfiles:
                ...
                if (load_profile_data and allowed_file(foundfile.filename)) or load_hpc_data or load_perf_data:
                    ...
                    if load_hpc_data:
                        if not os.path.exists(os.path.join(GlobalData.local_data, foundfile.filename)):
                            path_to_dir = pathlib.Path(
                                pathlib.Path(GlobalData.local_data) / pathlib.Path(foundfile.filename)).parent
                        ...
                        if is_hpc_experiment_file(foundfile.filename):
                            experiment_file = str(pathlib.Path(pathlib.Path(GlobalData.local_data) /
            ...
            if len(perf_data_files) > 0:
                ...
                results = \
                    jobhandler.convert_perf_data(perf_data_files, GlobalData.local_data, perf_working_directory)
            ...
            if experiment_file:
                ...
                if re.match(".*Line", analysis_level):
                    ...
                    include_loops = False
                    include_statements = False
                hpc_experiment = HPCExperimentHandler(GlobalData.local_data, experiment_file)
                ...
                GlobalData.hpc_results.append(HPCResultsHandler(GlobalData.local_data, results_file))
                ...
                for results_file in GlobalData.results_files:
                    ...
                    if is_hpc_result(full_path):
                        GlobalData.hpc_results\
                            .append(HPCResultsHandler(GlobalData.local_data, results_file))
                        main_logger.info(u"Loaded HPC Experiment results: " + results_file)
            ...
            GlobalData.processes, raw_events = get_results_info(GlobalData.local_data,
                                                                      GlobalData.results_files)
            GlobalData.trace_jobs = get_trace_jobs(GlobalData.local_data, GlobalData.results_files)
            GlobalData.cpu = get_cpu(GlobalData.local_data, GlobalData.results_files)
            GlobalData.loaded_cpu_definition = get_cpu_definition(GlobalData.cpu, raw_events)
            loaded_events = GlobalData.loaded_cpu_definition.get_active_events()
            GlobalData.jobs = get_jobs(GlobalData.results_files)
            GlobalData.enabled_modes = GlobalData.loaded_cpu_definition.get_enabled_modes()
...
def run_summary():
    ...
    layout["footer"] = "Loaded Results: " + " & ".join(layout["Results"])
    event_counters, run_numbers, run_durations, run_parameters = get_run_summary(GlobalData.local_data,
                                                                                 GlobalData.results_files)
    run_summary_table = ""
    if len(GlobalData.results_files)>0:
        run_summary_table = generate_run_summary_table(GlobalData.loaded_cpu_definition, event_counters,
