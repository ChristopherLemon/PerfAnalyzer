...
AnalysisView = Blueprint(
    "AnalysisView", __name__, template_folder="templates", static_folder="static"
)
...
def general_analysis():
    ...
    analysis_model.cluster_labels = run_analysis(
        analysis_data, event1, event2, centred, append_cluster_labels, log_scale
    )
    # Prepare plots
    ...
    analysis_model.layout.flamegraph = get_flamegraph(
        analysis_data, analysis_model.process_list, analysis_model.flamegraph_mode
    )
...
def initialise_analysis_model_cluster_data(analysis_data):
    events_dict = OrderedDict(
        [
            (raw_event_to_event(event, GlobalData.loaded_cpu_definition), event)
            for event in analysis_data.get_events()
        ]
    )
...
def update_analysis_model_base_event_data(base_event, events):
    ...
    analysis_model.system_wide = all_stack_data[
        base_event
    ].get_system_wide_mode_enabled()
    analysis_model.base_event_selected_ids = all_stack_data[
        base_event
    ].get_biggest_process_ids_for_each_job()
    ...
    analysis_model.base_event_reference_id = (
        all_stack_data[base_event].get_base_case_id().label
    )
...
def update_analysis_model_process_data(process):
    ...
    for process_id in ids:
        ...
        if (
            job == analysis_model.reference_job
            and pid == analysis_model.reference_pid
            and tid == analysis_model.reference_tid
            and event == analysis_model.reference_event
        ):
            ...
            if process_id.event_type == "custom_event_ratio":
                analysis_model.reference_count = float(process_id.count2) / float(
                    process_id.count1
                )
            else:
    ...
    for process_id in ids:
        for selected_id in analysis_model.base_event_selected_ids:
            if (
                process_id.job == selected_id.job
                and process_id.pid == selected_id.pid
                and process_id.tid == selected_id.tid
            ):
                event = process_id.event_name
...
def get_new_chart():
    ...
    if analysis_model.scatter_plot_type == "clusters":
        ...
        scatter_plot = get_hotspot_scatter_plot(
            analysis_data,
            event1,
            event2,
            svgchart,
...
def update_all_charts():
    ...
    event1 = analysis_model.event1
    event2 = analysis_model.event2
    raw_event1 = event_to_raw_event(
        analysis_model.event1, GlobalData.loaded_cpu_definition
    )
    raw_event2 = event_to_raw_event(
        analysis_model.event2, GlobalData.loaded_cpu_definition
    )
    centred = analysis_model.centred_scatter_plot == "centred"
    append_cluster_labels = analysis_model.flamegraph_mode == "clusters"
    log_scale = analysis_model.log_scale
...
def update_flamegraph_mode():
    ...
    analysis_model.layout.flamegraph = get_flamegraph(
        analysis_data, process_list, analysis_model.flamegraph_mode
    )
    return jsonify(analysis_model.layout.to_dict())
...
def get_source_code(symbol, label, get_source_table=True):
    ...
    for i in range(len(GlobalData.hpc_results)):
        if job_id == GlobalData.hpc_results[i].get_job_id():
            process_id = all_stack_data[
                analysis_model.reference_process
            ].get_process_id_from_label(label)
            source_code_info = generate_source_code_info(
                all_stack_data[analysis_model.reference_process],
                process_id,
                symbol,
...
def get_analysis(analysis_type):
    ...
    cluster_events = {
        "All": [],
        "Ratios": [],
    }  # Re-populate, as custom events may have been added
    for event in GlobalData.loaded_cpu_definition.get_active_events():
        if re.match(".* / .*", event):
            e1, _, e2 = event.partition(" / ")
            cluster_events["Ratios"].append(
                [
                    event_to_raw_event(e1, GlobalData.loaded_cpu_definition),
                    event_to_raw_event(e2, GlobalData.loaded_cpu_definition),
                ]
            )
        else:
            cluster_events["All"].append(
                event_to_raw_event(event, GlobalData.loaded_cpu_definition)
            )
...
def get_cluster_plot(
    ...
    raw_event1 = event_to_raw_event(event1, GlobalData.loaded_cpu_definition)
    raw_event2 = event_to_raw_event(event2, GlobalData.loaded_cpu_definition)
    chart = svg_chart.generate_cluster_plot(
        analysis_data,
        analysis_model.process_list,
        raw_event1,
        raw_event2,
