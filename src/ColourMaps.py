...
def get_gradient_colours(n_pos, n_neg, return_hex=True):
    ...
    if return_hex:
        c = [
            rgb2hex(colours[i][0], colours[i][1], colours[i][2])
...
def get_cluster_plot_colours(return_hex=True):
    ...
    if return_hex:
        ...
        c = [
            hex2rgb(cluster_plot_colours[i])
            for i in range(0, len(cluster_plot_colours))
        ]
    return c
...
def get_distinct_colours(return_hex=True):
    ...
    if return_hex:
        c = [
            rgb2hex(
                distinct_colours[i][0], distinct_colours[i][1], distinct_colours[i][2]
            )
            for i in range(0, len(distinct_colours))
        ]
    else:
        c = distinct_colours
    return c
