...
from src.ColourMaps import (
    get_gradient_colours,
    get_top_ten_colours,
...
class ChartWriter:
    ...
    def generate_timechart(
        self, stack_data, user_minx, user_maxx, title="", event_type="all"
    ):
        ...
        for process_id in ids:
            ...
            if event_type == chart_type or event_type == "all":
                nseries += 1
                x, y = self.restrict_xy(
                    stack_data.X[task_id][pid][tid],
    ...
    def generate_scatter_plot(
        self, stack_data, event1, event2, title="", event_type="all"
    ):
    ...
    def generate_hotspot_scatter_plot(
        self,
        analysis_data,
        process_list,
        reference_process,
                    ...
                    if node in data[job][pid][tid]:
                        ...
                        if abs(xi) > 0.0 or abs(yi) > 0.0 or nzeros == 1:
                            ...
                            if node in hotspot_colour_map:
                                ...
                                plot_data[i].append(
                                    {"value": (xi, yi), "label": label, "color": colour}
                                )
                            else:
                                plot_data[0].append(
                                    {
        ...
        if len(plot_data[0]) > 0:
            ...
            plot_data[0] = self.restrict_scatter(
                plot_data[0], minx, maxx, miny, maxy, nmax
            )
        if centred:
            xr = max(abs(minx), abs(maxx))
            yr = max(abs(miny), abs(maxy))
    ...
    @staticmethod
    def generate_horizontal_stacked_bar_chart(
        stack_data, start=1, number_to_rank=11, title=""
    ):
            ...
            for job in plot_data:
                ...
                for dummy_job in plot_data:
                    if job == dummy_job:
                        data_max[job].append(
                            {
                                "value": plot_data[job][s]["max"] - mean,
                                "label": plot_labels[job][s]["max"] + ": " + s,
                            }
                        )
                        data_mean[job].append(
                            {
                                "value": mean - plot_data[job][s]["min"],
                                "label": job + ": " + s,
                            }
                        )
                        data_min[job].append(
                            {
                                "value": plot_data[job][s]["min"],
                                "label": plot_labels[job][s]["min"] + ": " + s,
                            }
                        )
        ...
        data_max = []
        x_labels = []
        chart_table = pygal.HorizontalStackedBar(
            style=custom_style_hotspots,
            truncate_label=5,
            x_label_rotation=25,
            height=500,
            ...
            if output_event_type == event_type or output_event_type == "any":
                if pid in totals[task_id]:
                    if tid in totals[task_id][pid]:
                        ...
                        data.append(
                            {"value": totals[task_id][pid][tid], "label": label}
                        )
    ...
    def generate_bar_chart_multiple_jobs(
        self, all_stack_data, process_list, title="", output_event_type="any"
    ):
    ...
    def generate_vertical_stacked_bar_chart(
        self,
        stack_data,
        start=1,
        number_to_rank=11,
            ...
            if event_type == output_event_type or output_event_type == "any":
                if pid in totals[task_id]:
                    if tid in totals[task_id][pid]:
                        ...
                        if event_type == "original":
                            x_data = stack_data.get_original_event_stack_data(
                                process_id
                            )
                            x = {}
                            for stack in x_data:
                            ...
                            (
                                x_data,
                                y_data,
                            ) = stack_data.get_custom_event_ratio_stack_data(process_id)
                            x = {}
                            for stack in x_data:
                                node = stack.rpartition(";")[2]
                                if node not in x:
        ...
        if n_events > 0:
            ...
            for v in sorted_vals[start_index:end_index]:
                ...
                for label in plot_data[s]:
                    data.append(
                        {"value": plot_data[s][label], "label": label + ": " + s}
                    )
                chart.add(s, data)
            data = []
            for label in plot_data["other"]:
                data.append(
                    {"value": plot_data["other"][label], "label": label + ": other"}
                )
    ...
    def generate_vertical_stacked_bar_chart_multiple_jobs(
        self,
        all_stack_data,
        process_list,
        reference_process,
                        ...
                        if tid in totals[task_id][pid]:
                            ...
                            if event_type == "original":
                                ...
                                (
                                    x_data,
                                    y_data,
                                ) = stack_data.get_custom_event_ratio_stack_data(
                                    process_id
                                )
                                x = {}
                                for stack in x_data:
        ...
        if n_events > 0:
            ...
            for v in sorted_vals[start_index:end_index]:
                ...
                for label in event_ordered_plot_data[s]:
                    data.append(
                        {
                            "value": event_ordered_plot_data[s][label],
                            "label": label + ": " + s,
                        }
                    )
                chart.add(s, data)
            ...
            for label in event_ordered_plot_data["other"]:
                data.append(
                    {
                        "value": event_ordered_plot_data["other"][label],
                        "label": label + ": other",
                    }
                )
            chart.add("other", data)
    ...
    def generate_vertical_stacked_bar_chart_diff(
        self, stack_data, number_to_rank=10, title=""
    ):
        ...
        base_case_other = stack_data.totals[base_case_task_id][base_case_pid][
            base_case_tid
        ]
        x_data = stack_data.get_original_event_stack_data(base_case_id)
        base_case_x = {}
        ...
        max_total = 0.0
        for process_id in [
            proc_id for proc_id in ids if proc_id.label != base_case_id.label
        ]:
            x_data = stack_data.get_original_event_stack_data(process_id)
            x = {}
            total = 0.0
            ...
            sorted_deltas = sorted(
                deltas.items(), key=operator.itemgetter(1), reverse=True
            )
            for v in sorted_deltas[0:number_to_rank]:
                s = v[0]
                if s not in top_nodes:
                    top_nodes.append(s)
            for v in sorted_deltas[
                len(sorted_deltas) - 1 : len(sorted_deltas) - number_to_rank - 1 : -1
            ]:
                s = v[0]
            ...
            if pid in totals[task_id]:
                if tid in totals[task_id][pid]:
                    ...
                    for s in top_nodes:
                        ...
                        if s in deltas:
                            if deltas[s] >= 0.0:
                                plot_data[s + ": positive diff"][unique_label] = deltas[
                                    s
                                ]
                                ...
                                plot_data[s + ": negative diff"][unique_label] = deltas[
                                    s
                                ]
                ...
                plot_data = OrderedDict(
                    sorted(
                        plot_data.items(),
                        key=lambda xi: plot_data_sort(xi, max_total),
                        reverse=True,
                    )
                )
                n_pos = 0
        ...
        chart = pygal.StackedBar(
            style=custom_style_barchart_diff,
            truncate_label=5,
            x_label_rotation=25,
            height=500,
            show_legend=False,
    ...
    def generate_bar_chart_total_diff(self, stack_data, title=""):
        ...
        base_case_pid = base_case_id.pid
        base_case_tid = base_case_id.tid
        base_case_total = stack_data.totals[base_case_task_id][base_case_pid][
            base_case_tid
        ]
        x_labels = []
        nseries = 0
    ...
    def restrict_scatter_multiple(self, plot_data, xmin, xmax, ymin, ymax):
        ...
        if total_points > 20 * self.max_points:
            ...
            for i in plot_data:
                plot_data[i] = self.restrict_scatter(
                    plot_data[i], xmin, xmax, ymin, ymax, nmax
                )
        return plot_data
