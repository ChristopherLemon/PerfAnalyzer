...
def generate_run_summary_table(
    cpu_definition, event_counters, run_numbers, run_durations, run_parameters
):
    ...
    for job in event_counters:
        ...
        table_html += [
            "<th>" + job + " Events" "</th>",
            "<th>Perf Event</th>",
            "<th>Events Per Sample</th>",
            "<th>Run Number</th>",
            "<th>Run Duration (S)</th>",
            "<th>CPU</th>",
            "<th>System Wide Profiling</th>",
        ...
        for raw_event, run_number in runs:
            if run_number in run_durations[job]:
                ...
                table_html.append(
                    "<td>" + str(event_counters[job][raw_event]) + "</td>"
                )
                table_html.append("<td>" + str(run_numbers[job][raw_event]) + "</td>")
                max_run_duration = max(run_durations[job].values())
                pc = round_sig(
                    100.0 * run_durations[job][str(run_number)] / max_run_duration, 2
                )
                table_html.append(
                    "<td>"
                    + str(run_durations[job][str(run_number)])
                    + " ("
                    + str(pc)
                    + " %)"
                    + "</td>"
                ...
                table_html.append(
                    "<td>" + str(run_parameters[job]["time_interval"]) + "</td>"
                )
                table_html.append("</tr>")
        table_html.append("</table>")
