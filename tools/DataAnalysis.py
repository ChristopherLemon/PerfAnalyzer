...
class RooflineAnalysis:
    ...
    def set_events(self, roofline_events):
        self.roofline_events = roofline_events
        self.events = roofline_events['All']
        ...
        for job in self.cluster_data:
            for pid in self.cluster_data[job]:
                for tid in self.cluster_data[job][pid]:
                    for node in self.cluster_data[job][pid][tid]:
                        for i in range(0, 2):
                            x = self.cluster_data[job][pid][tid][node][i]
                            if x > 0.0:
                                self.cluster_data[job][pid][tid][node][i] = log10(x)
    ...
    def add_offset(self, reference_process):
        ...
        for node in self.cluster_data[job][pid][tid]:
            if node not in self.base_case_offset:
                self.base_case_offset[node] = [0.0, 0.0]
            for i in range(0, 2):
                self.base_case_offset[node][i] = self.cluster_data[job][pid][tid][node][i]
        for job in self.cluster_data:
            for pid in self.cluster_data[job]:
                for tid in self.cluster_data[job][pid]:
                    for node in self.cluster_data[job][pid][tid]:
                        if node in self.base_case_offset:
                            for i in range(0, 2):
                                offset = self.base_case_offset[node][i]
                                self.cluster_data[job][pid][tid][node][i] -= offset
    ...
    def make_data(self, reference_process, centred=False, log_scale=False):
        ...
        for stack_data in self.all_stack_data.values():
            ...
            for process_id in ids:
                ...
                tid = process_id.tid
                raw_event = process_id.raw_event
                if raw_event in self.roofline_events["All"]:
                    if job not in self.data:
                        self.data[job] = {}
                        self.counter[job] = {}
                    if pid not in self.data[job]:
                    ...
                    for stack in counts:
                        ...
                        s = re.sub(job + ";", "", s)
                        if s not in self.data[job][pid][tid]:
                            self.data[job][pid][tid][s] = {e:0.0 for e in self.events}
                        self.data[job][pid][tid][s][raw_event] += counts[stack]
        ...
        for job in self.data:
            if job not in flops:
                ...
                time[job] = {}
            for pid in self.data[job]:
                if pid not in flops[job]:
                    flops[job][pid] = {}
                    loads[job][pid] = {}
                    time[job][pid] = {}
                for tid in self.data[job][pid]:
                    if tid not in flops[job][pid]:
                        flops[job][pid][tid] = {}
                        loads[job][pid][tid] = {}
                        time[job][pid][tid] = {}
                    for s in self.data[job][pid][tid]:
                        node = s.rpartition(";")[2]
                        if node not in flops[job][pid][tid]:
                            flops[job][pid][tid][node] = 0.0
                            loads[job][pid][tid][node] = 0.0
                            time[job][pid][tid][node] = 0.0
                        for e in self.roofline_events['Memory']:
                            loads[job][pid][tid][node] += float(self.data[job][pid][tid][s][e])
                        for e in self.roofline_events['Flops']:
                            flops[job][pid][tid][node] += float(self.data[job][pid][tid][s][e])
                        e = self.roofline_events['Time'][0]
                        time[job][pid][tid][node] += float(self.data[job][pid][tid][s][e])
        for job in flops:
            if job not in self.X:
                self.X[job] = {}
                self.Y[job] = {}
            for pid in flops[job]:
                if pid not in self.X[job]:
                    self.X[job][pid] = {}
                    self.Y[job][pid] = {}
                for tid in flops[job][pid]:
                    if tid not in self.X[job][pid]:
                        self.X[job][pid][tid] = {}
                        self.Y[job][pid][tid] = {}
                    for node in flops[job][pid][tid]:
                        if node not in self.X[job][pid][tid]:
                            self.X[job][pid][tid][node] = 0.0
                            self.Y[job][pid][tid][node] = 0.0
                        r = 0.0
                        if time[job][pid][tid][node] > 0.0:
                            r = loads[job][pid][tid][node]/time[job][pid][tid][node]
                            rr = flops[job][pid][tid][node]/time[job][pid][tid][node]
                            self.max_fops = max(self.max_fops, rr)
                        if r > 0.01:
                            self.max_bandwidth = max(self.max_bandwidth, r)
                            self.X[job][pid][tid][node] = flops[job][pid][tid][node] / loads[job][pid][tid][node]
                            self.Y[job][pid][tid][node] = flops[job][pid][tid][node] / time[job][pid][tid][node]
                        else:
                            self.X[job][pid][tid][node] = 0.0
                            self.Y[job][pid][tid][node] = 0.0
        ...
        for job in self.X:
            if job not in self.cluster_data:
                self.cluster_data[job] = {}
            for pid in self.X[job]:
                if pid not in self.cluster_data[job]:
                    self.cluster_data[job][pid] = {}
                for tid in self.X[job][pid]:
                    if tid not in self.cluster_data[job][pid]:
                        self.cluster_data[job][pid][tid] = {}
                    for node in self.X[job][pid][tid]:
                        if node not in self.cluster_data[job][pid][tid]:
                            self.cluster_data[job][pid][tid][node] = {}
                        x = [self.X[job][pid][tid][node], self.Y[job][pid][tid][node]]
                        self.cluster_data[job][pid][tid][node] = x
    ...
    def setup_cluster_analysis(self, xlower=None, xupper=None, ylower=None, yupper=None):
        n = 0
        self.cluster_map = {}
        self.cluster_analysis.blob = []
        for job in self.cluster_data:
            ...
            for pid in self.cluster_data[job]:
                ...
                for tid in self.cluster_data[job][pid]:
                    ...
                    for node in self.cluster_data[job][pid][tid]:
                        if xlower:
                            count1 = self.cluster_data[job][pid][tid][node][0]
                            count2 = self.cluster_data[job][pid][tid][node][1]
                            if count1 < ylower or count1 > yupper or count2 < xlower or count2 > xupper:
                                continue
                        if node not in self.cluster_map[job][pid][tid]:
                            self.cluster_map[job][pid][tid][node] = n
                        x = [self.cluster_data[job][pid][tid][node][0], self.cluster_data[job][pid][tid][node][1]]
                        self.cluster_analysis.blob.append(x)
                        n += 1
    ...
    def make_stack_map(self, clusters, append_cluster_labels, event1=None, event2=None, xlower=None, xupper=None, ylower=None, yupper=None):
        self.cluster_flamegraph.make_stack_map(self.all_stack_data, clusters, append_cluster_labels, 1, 0, xlower, xupper, ylower, yupper)

    def calculate_ratios(self, n, event1=None, event2=None, xlower=None, xupper=None, ylower=None, yupper=None):
        self.setup_cluster_analysis(xlower, xupper, ylower, yupper)
        self.cluster_analysis.calculate_ratios(n)
        self.cluster_filter = [i for i in range(0, n)]
        self.cluster_flamegraph.add_data(self.cluster_data, self.cluster_analysis.cluster_labels, self.cluster_map)
        self.initialised = True
                        ...
                        for node in self.cluster_data[job][pid][tid]:
                            ...
                            if i in self.cluster_filter:
                                if node not in self.data:
                                    data[job][pid][tid][node] = {}
                                for i in range(0,2):
                                    data[job][pid][tid][node][i] = self.cluster_data[job][pid][tid][node][i]
            return data
        else:
            return self.cluster_data

    def get_cluster_events(self):
        return self.roofline_events
